...
function profiler_report ($url_params,
  ...
  if ($diff_mode) {
    $run_delta = xhprof_compute_diff($run1_data, $run2_data);
	  $overall_totals = (array)$totals;
	  $symbol_tab  = xhprof_compute_flat_info($run_delta, $overall_totals);
	  $overall_totals1 = (array)$totals_1;
	  $symbol_tab1 = xhprof_compute_flat_info($run1_data, $overall_totals1);
	  $overall_totals2 = (array)$totals_2;
	  $symbol_tab2 = xhprof_compute_flat_info($run2_data, $overall_totals2);
  } else {
	  $overall_totals3 = (array)$totals;
	  $symbol_tab = xhprof_compute_flat_info($run1_data, $overall_totals3);
  }
